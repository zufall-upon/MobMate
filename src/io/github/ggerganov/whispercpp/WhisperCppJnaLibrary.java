...
public interface WhisperCppJnaLibrary extends Library {
    WhisperCppJnaLibrary instance = Native.load("Whisper", WhisperCppJnaLibrary.class, Collections.singletonMap(Library.OPTION_STRING_ENCODING, "UTF-8"));
    ...
    /**
     * Allocate (almost) all memory needed for the model by loading from a file without allocating
     * the state.
     *
     * @param path_model Path to the model file
     * @return Whisper context on success, null on failure
    ...
    /**
     * Run the Whisper decoder to obtain the logits and probabilities for the next token. Make sure
     * to call whisper_encode() first. tokens + n_tokens is the provided context for the decoder.
     * n_past is the number of tokens to use from previous decoder calls. Returns 0 on success TODO:
     * add support for multiple decoders
     */
     ...
     * resulting tokens. Returns the number of tokens on success, no more than n_max_tokens Returns
     * -1 on failure TODO: not sure if correct
     */
    ...
    /**
     * @return the short string of the specified language id (e.g. 2 -> "de"), returns nullptr if
     *         not found
     */
    ...
    /**
     * Token logits obtained from the last call to whisper_decode(). The logits for the last token
     * are stored in the last row Rows: n_tokens Cols: n_vocab
     */
    ...
    // Note: Even if `whisper_full_params is stripped back to just 4 ints, JNA throws "Invalid
    // memory access"
    ...
    /**
     * Run the entire model: PCM -> log mel spectrogram -> encoder -> decoder -> text Not thread
     * safe for same context Uses the specified decoding strategy to obtain the text.
     */
